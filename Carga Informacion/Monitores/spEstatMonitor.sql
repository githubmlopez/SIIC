...
IF  EXISTS( SELECT 1 FROM CARGADOR.sys.procedures WHERE Name =  'spEstatMonitor')
BEGIN
  DROP  PROCEDURE spEstatMonitor
END
GO

CREATE PROCEDURE [dbo].[spEstatMonitor]  
(
...
--  SELECT * FROM @TProcMonitor
  ...
  BEGIN
    ...
    IF  EXISTS(SELECT  1 FROM  FC_GEN_TAREA t
               WHERE  t.ID_CLIENTE     =  @pIdCliente    AND
			          t.CVE_EMPRESA    =  @pCveEmpresa   AND 
					  t.CVE_APLICACION =  @pCveAplicacion AND
    ...
    BEGIN
      ...
      WHERE  t.ID_CLIENTE     =  @pIdCliente    AND
  	  ...
  	  IF  NOT EXISTS(SELECT  1  FROM  FC_GEN_TAREA_EVENTO te
	             WHERE  te.ID_CLIENTE      =  @pIdCliente     AND
				        te.CVE_EMPRESA     =  @pCveEmpresa    AND
                        te.CVE_APLICACION  =  @pCveAplicacion AND
