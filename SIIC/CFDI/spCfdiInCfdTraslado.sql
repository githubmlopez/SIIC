...
SET NOCOUNT ON
GO
IF  EXISTS( SELECT 1 FROM ADMON01.sys.procedures WHERE Name =  'spCfdInCfdTraslado')
BEGIN
  DROP  PROCEDURE spCfdInCfdTraslado
  END
GO
--EXEC spCfdInCfdTraslado 1,'CU','MARIO','SIIC','202008',202,1,1,'P',' ',' ',1,' ',' '
CREATE PROCEDURE dbo.spCfdInCfdTraslado
(
@pIdCliente       int,
...
----------------------------------------------------------------------------------------------------------------------
-- Carga de la infromación de Traslado                                                                --
----------------------------------------------------------------------------------------------------------------------

  SET @seccion  =  'Traslado'
	
  INSERT INTO CFDI_TRASLADO
 (
  CVE_EMPRESA,
  ANO_MES,
  CVE_TIPO,
  ...
  FROM OPENXML(@pHDoc, 'cfdi:Comprobante/cfdi:Conceptos/cfdi:Concepto/cfdi:Impuestos/cfdi:Traslados/cfdi:Traslado',2)
  WITH 
 (
  ID_NODO      int '@mp:id',
