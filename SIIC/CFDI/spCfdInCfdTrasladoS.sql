...
SET NOCOUNT ON
GO
iF  EXISTS( SELECT 1 FROM ADMON01.sys.procedures WHERE Name =  'spCfdInCfdTrasladoS')
BEGIN
  DROP  PROCEDURE spCfdInCfdTrasladoS
END
GO
-- EXEC spCfdInCfdTrasladoS 1,'CU','MARIO','SIIC','202008',202,1,1,'P',' ',' ',0,' ',' '
CREATE PROCEDURE dbo.spCfdInCfdTrasladoS
(
@pIdCliente       int,
...
----------------------------------------------------------------------------------------------------------------------
  ...
  SET @seccion  =  'TrasladoS'

  INSERT INTO CFDI_TRASLADO_S
 (
  CVE_EMPRESA,
  ANO_MES,
  CVE_TIPO,
  ...
  ID_NODO,
  ID_NODO_P
  FROM OPENXML(@pHdoc, 'cfdi:Comprobante/cfdi:Conceptos/cfdi:Concepto/cfdi:Impuestos/cfdi:Traslados',2)
  WITH 
 (
  ID_NODO   INT '@mp:id',
