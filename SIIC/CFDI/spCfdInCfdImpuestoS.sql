...
SET NOCOUNT ON
GO
IF  EXISTS( SELECT 1 FROM ADMON01.sys.procedures WHERE Name =  'spCfdInCfdImpuestoS')
BEGIN
  DROP  PROCEDURE spCfdInCfdImpuestoS
END
GO
--EXEC spCfdInCfdImpuestoS 1,'CU','MARIO','SIIC','202008',202,1,1,'P',' ',' ',0,' ',' '
CREATE PROCEDURE dbo.spCfdInCfdImpuestoS
(
@pIdCliente       int,
...
----------------------------------------------------------------------------------------------------------------------
  ...
  SET @seccion  =  'ImpuestoS'

  INSERT INTO CFDI_IMPUESTO_S
 (
  CVE_EMPRESA,
  ANO_MES,
  CVE_TIPO,
  ...
  ID_NODO,
  ID_NODO_P
  FROM OPENXML(@pHdoc, 'cfdi:Comprobante/cfdi:Conceptos/cfdi:Concepto/cfdi:Impuestos',2)
  WITH 
(
  ID_NODO   INT '@mp:id',
