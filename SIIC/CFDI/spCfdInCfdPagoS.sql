...
SET NOCOUNT ON
GO
IF  EXISTS( SELECT 1 FROM ADMON01.sys.procedures WHERE Name =  'spCfdInCfdPagoS')
BEGIN
  DROP  PROCEDURE spCfdInCfdPagoS
END
GO
--EXEC spCfdiBaseDatos 1,'CU','MARIO','SIIC','202008',202,1,1,'P',' ',' ',0,' ',' '
CREATE PROCEDURE dbo.spCfdInCfdPagoS
(
@pIdCliente       int,
...
----------------------------------------------------------------------------------------------------------------------
 ...
 (
  ...
  ID_NODO,
  ID_NODO_P
  FROM OPENXML(@pHdoc, 'cfdi:Comprobante/cfdi:Complemento/pago10:Pagos',2)
  WITH 
(
  ID_NODO   INT '@mp:id',
