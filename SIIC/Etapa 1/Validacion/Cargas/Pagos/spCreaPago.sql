...
SET NOCOUNT ON
GO
IF  EXISTS( SELECT 1 FROM ADMON01.sys.procedures WHERE Name =  'spCreaPago')
BEGIN
  DROP  PROCEDURE spCreaPago
END
GO

--EXEC spCreaPago 'CU','MARIO','201906',146,8137,' ',' '
CREATE PROCEDURE [dbo].[spCreaPago]
(
@pCveEmpresa     varchar(4),
@pCveChequera    varchar(6),
@pBenefPago      varchar(100),
@pFProgPago      date,
@pCveMetodoPago  varchar(3),
@TMovCxCPag TMOVCXCPAG READONLY,
...
-----------------------------------------------------------------------------------------------------
	...
	USO_CFDI_REC,
	REG_FISCAL_EMI
    FROM   CFDI_COMPROBANTE c, @TMovCxCPag t, CFDI_EMISOR e, CFDI_RECEPTOR r WHERE
	c.CVE_EMPRESA = t.CVE_EMPRESA AND
	c.ANO_MES     = t.ANO_PERIODO AND
	c.CVE_TIPO    = t.CVE_TIPO    AND
	c.UUID        = t.UUID        AND
...
----------------------------------
	...
	SET @folio_pago = (select  NUM_FOLIO FROM  CI_FOLIO  WHERE CVE_FOLIO  =  @k_pago)
	UPDATE CI_FOLIO SET NUM_FOLIO = NUM_FOLIO + 1  WHERE CVE_FOLIO  =  @k_pago
   ...
   (
    ...
    BEGIN
	  ...
	  EXEC spCFDICalImpto @cve_empresa, @pCodigoUsuario, @ano_mes , @pIdProceso, @pIdTarea, @cve_tipo, @uuid, 
...
-----------------------------------------------------------------------------------------------------
      ...
      BEGIN
	    ...
	    SET  @cve_operacion = dbo.fnObtCveOper(@cve_empresa, @ano_periodo, @cve_tipo, @uuid, @cve_prod_serv)
