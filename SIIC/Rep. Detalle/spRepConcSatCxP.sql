...
ALTER PROCEDURE [dbo].[spRepConcSatCxP] 
...
BEGIN
  ...
  CASE
  WHEN CVE_CONC_MAN IS NOT NULL
  THEN CVE_CONC_MAN
  WHEN SIT_CONCILIA = 'CO' 
  THEN '*CO'
  WHEN SIT_CONCILIA = 'NC'
  THEN '*NC'
  ELSE CVE_CONC_MAN
  END as 'CVE. CONC. MAN'
  FROM CI_SAT_CXP EFECTO_COMPROB WHERE ISNULL(EFECTO_COMPROB,' ') NOT IN  (@k_cve_no_conc1,@k_cve_no_conc2) ORDER BY ANO_MES_PROC
END
