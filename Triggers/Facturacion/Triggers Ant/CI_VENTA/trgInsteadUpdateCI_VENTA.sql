USE [ADMON01]
GO
/****** Object:  Trigger [dbo].[rgInsteadUpdateCI_VENTA]     ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
-- Create trigger on table CI_VENTA for Update statement
ALTER TRIGGER [dbo].[trgInsteadUpdateCI_VENTA] ON [dbo].[CI_VENTA]
INSTEAD OF UPDATE
AS
...
declare
    @id_venta_d        int,
    @id_cliente_d      numeric(10),
    @id_cliente_r_d    numeric(10)
...
select   @id_venta_d        =  d.ID_VENTA from inserted d;
select   @id_cliente_d      =  d.ID_CLIENTE from inserted d;
select   @id_cliente_r_d    =  d.ID_CLIENTE_R from inserted d;


set  @b_reg_correcto =  @k_verdadero
set  @tx_error_part  =  ' '

IF   NOT EXISTS (SELECT 1 FROM CI_VENTA  WHERE   ID_VENTA  =  @id_venta)
BEGIN
  set @b_reg_correcto   =  @k_falso;
  set @tx_error_part    =  @tx_error_part + ': No existe el registro de la venta';
END
...
BEGIN
    select ' id cliente ' + convert(varchar(10),@id_cliente) 
    UPDATE CI_VENTA 
           SET ID_CLIENTE   = @id_cliente,
               ID_CLIENTE_R = @id_cliente_r where ID_VENTA = @id_venta
END
ELSE
BEGIN
