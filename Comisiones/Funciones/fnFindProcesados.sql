CREATE FUNCTION fnFindProcesados (@cve_empresa varchar(4), @serie varchar(6),
                                  @id_cxc int,@id_item int)
RETURNS bit
...
BEGIN
  ...
  IF EXISTS   (SELECT 1 
                ...
                WHERE   
--              join Cupones Pagados con parametros de entrada
                i.CVE_EMPRESA      = @cve_empresa                              AND
                i.SERIE            = @serie                                    AND
                i.ID_CXC           = @id_cxc                                   AND
                i.ID_ITEM          = @id_item                                  AND
--              join Factura - Item 
